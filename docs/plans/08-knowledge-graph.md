...
entities: defineTable({
  ...
  name: v.string(),
  slug: v.string(),                          // "person:bret-taylor"
...
│   └── ...
├── quotes/
│   └── bret-enterprise-first-2026-04.md
├── events/
...
**Note:** when user opens the Signal repo root as an Obsidian vault, both `knowledge/` and `battlecards/` become part of the same graph. Wikilinks work across folders. `[[bret-taylor]]` in a battlecard resolves to `knowledge/people/bret-taylor.md`.
...
Stable `slug` format: `<type>:<normalized-name>`, e.g., `person:bret-taylor`, `company:sutter-health`.
...
## Notable quotes
- [[bret-enterprise-first-2026|"we're built enterprise-first from day one"]]
...
npm run sync:vault -- --slug=person:bret-taylor   # Single entity
...
- Navigate to `knowledge/people/bret-taylor.md`
