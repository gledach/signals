...
try {
  execSync('node --env-file-if-exists=.env db-migrate.mjs', { stdio: 'inherit', env: process.env });
} catch { /* migration failure is non-fatal for existing DBs */ }

const { logCronStart, logCronFinish } = await import('./store.mjs');
...
run('fetch signals',       'node --env-file-if-exists=.env fetch-signals.mjs');
run('watch:hn',            'node --env-file-if-exists=.env hn-watch.mjs');
run('watch:sites',         'node --env-file-if-exists=.env sitemap-watch.mjs');
run('watch:certs',         'node --env-file-if-exists=.env cert-watch.mjs');
run('watch:youtube',       'node --env-file-if-exists=.env youtube-watch.mjs');
run('watch:tavily',        'node --env-file-if-exists=.env tavily-watch.mjs');
run('watch:trends',        'node --env-file-if-exists=.env trends-watch.mjs');
run('correlate',           'node --env-file-if-exists=.env correlate.mjs');
run('refresh battlecards', 'node --env-file-if-exists=.env refresh-battlecards.mjs');

// ── Daily: morning brief (06:00 UTC run only) ──────────────────────────────
const isDailyRun = hour >= 5 && hour <= 7; // catch the ~06:00 UTC window

if (isDailyRun) {
  run('daily brief', 'node --env-file-if-exists=.env analyst.mjs --mode=brief --force');
  run('daily scan',  'node --env-file-if-exists=.env analyst.mjs --mode=scan --force');
}
...
if (isWeeklyRun) {
  run('deep analysis (all competitors)', 'node --env-file-if-exists=.env analyst.mjs --mode=deep --all-competitors --force');
  run('weekly report',                    'node --env-file-if-exists=.env weekly-report.mjs');
}
